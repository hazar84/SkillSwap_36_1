...
export const SkillPage: React.FC = () => {
	...
	const viewedUser = React.useMemo(
	...
	const isAlreadyProposed = useMemo(() => {
		...
		return sentExchanges.some(
			(exchange) =>
				exchange.toUserId === viewedUser.id &&
				exchange.skillId === viewedUser.skillCanTeach.id
		)
	}, [sentExchanges, viewedUser])
	...
	const categoryId =
		subcategoryToCategoryMap[viewedUser.skillCanTeach.subcategoryId]
	...
	return (
		<>
			...
			<ModalUI isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
				<Success offer='notification' onClick={() => setIsModalOpen(false)} />
			</ModalUI>
			<FooterUI />
		</>
	)
